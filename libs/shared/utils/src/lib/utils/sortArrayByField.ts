...
export const sortArrayByField = <T extends Record<string, unknown>>(
  ...
  const sortedObjects = [...objects].sort((a, b) => {
    const aValue =
      a[field] instanceof Date ? (a[field] as Date).getTime() : a[field];
    const bValue =
      b[field] instanceof Date ? (b[field] as Date).getTime() : b[field];
    ...
    if (
      ...
      throw new Error(
        'sortArrayByField only works with strings, numbers and dates'
      );
    }
